...
class PushManager {
  ...
  static Future<void> _showNotification(
    ...
    if (data != null) {
      ...
      final notNullData =
          data.map((key, value) => MapEntry(key as String, value as dynamic));

      _markAsDelivered(notNullData);
  ...
  static Future<void> _markAsDelivered(Map<String, dynamic> notNullData) async {
    runZonedGuarded(() async {
      if (!UIKit.isInitialized()) {
        await UIKit.initSendbirdUIKit();
      }

      if (UIKit.isInitialized()) {
        debugPrint('[PushManager][markAsDelivered()]');
        await SendbirdChat.markAsDelivered(data: notNullData);
      }
    }, (error, stack) {
      debugPrint('[PushManager][markAsDelivered()][Error] ${error.toString()}');
    });
  }
